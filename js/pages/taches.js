...
const TachesPage = {
  ...
  _reunionTypeColors: {
    equipe: '#3b82f6', direction: '#F5512E', operationnel: '#f97316',
    ...
    return `
      <div class="dash-section">
        <div class="dash-kpi-row">
          ...
          ${this._kpiCard('solar:chart-bold-duotone', '#F5512E', 'Taux complétion', tauxCompletion + '%', "TachesPage._kpiNav('kanban')")}
        </div>
      </div>
  ...
  _avatarBubble(name) {
    ...
    const colors = ['#F5512E', '#f97316', '#22c55e', '#ef4444', '#F5512E', '#3b82f6', '#ec4899'];
  ...
  _kanbanCard(t) {
    ...
    if (t.delegation && t.delegation.statut === 'transferee') {
      html += '<span style="display:inline-flex;align-items:center;gap:3px;padding:2px 8px;border-radius:6px;font-size:10px;font-weight:700;background:rgba(139,92,246,.12);color:#F5512E;"><iconify-icon icon="solar:hand-shake-bold-duotone" style="font-size:12px;"></iconify-icon> Déléguée par ' + Utils.escHtml(t.delegation.delegueParNom || '') + '</span>';
    }
  ...
  _viewTask(taskId) {
    ...
    if ((t.etiquettes || []).length > 0) {
      ...
      t.etiquettes.forEach(tag => {
        body += '<span style="background:rgba(245,81,46,.12);color:#F5512E;padding:2px 10px;border-radius:12px;font-size:11px;">' + Utils.escHtml(tag) + '</span>';
      });
      body += '</div></div>';
    }
    ...
    if (t.delegation) {
      ...
      const statusColors = { transferee: { bg: 'rgba(139,92,246,.08)', border: 'rgba(139,92,246,.2)', color: '#F5512E', label: 'Transférée', icon: 'solar:check-circle-bold-duotone' }, annulee_admin: { bg: 'rgba(239,68,68,.08)', border: 'rgba(239,68,68,.2)', color: '#ef4444', label: 'Annulée par admin', icon: 'solar:close-circle-bold-duotone' }, en_attente: { bg: 'rgba(139,92,246,.08)', border: 'rgba(139,92,246,.2)', color: '#F5512E', label: 'En attente', icon: 'solar:clock-circle-bold-duotone' } };
    ...
    let delegateBtn = '';
    if (canDelegate) {
      delegateBtn = '<button class="btn btn-sm" style="color:#F5512E;" onclick="TachesPage._openDelegateForm(\'' + t.id + '\')">'
  ...
  _openDelegateForm(taskId) {
    ...
    Modal.open({
      title: '<iconify-icon icon="solar:hand-shake-bold-duotone" style="color:#F5512E;"></iconify-icon> Déléguer la tâche',
      body: '<div style="padding:4px;">'
        ...
        + '<iconify-icon icon="solar:info-circle-line-duotone" style="color:#F5512E;"></iconify-icon> '
        ...
        + '</div>',
      footer: '<button class="btn" style="background:#F5512E;color:white;border:none;" onclick="TachesPage._confirmDelegate(\'' + taskId + '\')"><iconify-icon icon="solar:hand-shake-bold-duotone"></iconify-icon> Déléguer</button>'
  ...
  _getStyleContent() {
    return `
      ...
      .taches-fab {
        ...
        background:linear-gradient(135deg, #F5512E, #DE3E1E); color:#fff; border:none;
        cursor:pointer; display:flex; align-items:center; justify-content:center;
        box-shadow:0 4px 15px rgba(245,81,46,.4), 0 0 30px rgba(245,81,46,.15); transition:all .25s cubic-bezier(.4,0,.2,1); z-index:100;
      }
      .taches-fab:hover { transform:scale(1.08) rotate(90deg); box-shadow:0 8px 25px rgba(245,81,46,.5); }
      ...
      .dash-bar-fill { height:100%; border-radius:4px; background:linear-gradient(90deg, #F5512E, #DE3E1E); transition:width .3s ease; }
      ...
      .kanban-tag {
        background:rgba(245,81,46,.1); color:#8AA8FF; padding:1px 7px; border-radius:8px;
      ...
      .gantt-view-btn.active { background:rgba(245,81,46,.15); color:#8AA8FF; border-color:rgba(245,81,46,.3); font-weight:600; }
      ...
      .gantt-months {
        display:flex; background:rgba(245,81,46,0.08);
        border-bottom:1px solid rgba(255,255,255,0.04);
      }
      .gantt-month {
      ...
      .gantt-day.gantt-today-col { background:rgba(245,81,46,0.08); }
      ...
      .gantt-cell.gantt-today-col { background:rgba(245,81,46,0.04); }
      ...
      .reunion-date-day { font-size:1.2rem; font-weight:700; color:#F5512E; line-height:1; }
      .reunion-date-month { font-size:10px; color:#F5512E; text-transform:uppercase; font-weight:600; }
      ...
      .liste-bulk-bar {
        ...
        background:rgba(245,81,46,.08); border:1px solid rgba(245,81,46,.15); margin-bottom:12px;
