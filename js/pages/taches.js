...
const TachesPage = {
  ...
  _reunionTypeColors: {
    equipe: '#3b82f6', direction: '#E8543A', operationnel: '#f97316',
  ...
  render(container) {
    ...
    h1Icon.style.cssText = 'color:#E8543A;font-size:1.5rem;';
    ...
    return `
      <div class="dash-section">
        <div class="dash-kpi-row">
          ${this._kpiCard('solar:clipboard-list-bold-duotone', '#E8543A', 'Tâches actives', active.length, "TachesPage._kpiNav('liste')")}
          ...
          ${this._kpiCard('solar:chart-bold-duotone', '#E8543A', 'Taux complétion', tauxCompletion + '%', "TachesPage._kpiNav('kanban')")}
        </div>
      </div>
      ...
      <div class="dash-grid-2col">
        <div class="dash-card">
          <div class="dash-card-header">
            <iconify-icon icon="solar:users-group-rounded-bold-duotone" style="color:#E8543A;"></iconify-icon>
            Charge par membre
          </div>
  ...
  _avatarBubble(name) {
    ...
    const colors = ['#E8543A', '#f97316', '#22c55e', '#ef4444', '#E8543A', '#3b82f6', '#ec4899'];
  ...
  _kanbanCard(t) {
    ...
    if (t.delegation && t.delegation.statut === 'transferee') {
      html += '<span style="display:inline-flex;align-items:center;gap:3px;padding:2px 8px;border-radius:6px;font-size:10px;font-weight:700;background:rgba(139,92,246,.12);color:#E8543A;"><iconify-icon icon="solar:hand-shake-bold-duotone" style="font-size:12px;"></iconify-icon> Déléguée par ' + Utils.escHtml(t.delegation.delegueParNom || '') + '</span>';
    }
  ...
  _viewTask(taskId) {
    ...
    if ((t.etiquettes || []).length > 0) {
      ...
      t.etiquettes.forEach(tag => {
        body += '<span style="background:rgba(232,84,58,.12);color:#E8543A;padding:2px 10px;border-radius:12px;font-size:11px;">' + Utils.escHtml(tag) + '</span>';
      });
      body += '</div></div>';
    }
    ...
    if (t.delegation) {
      ...
      const statusColors = { transferee: { bg: 'rgba(139,92,246,.08)', border: 'rgba(139,92,246,.2)', color: '#E8543A', label: 'Transférée', icon: 'solar:check-circle-bold-duotone' }, annulee_admin: { bg: 'rgba(239,68,68,.08)', border: 'rgba(239,68,68,.2)', color: '#ef4444', label: 'Annulée par admin', icon: 'solar:close-circle-bold-duotone' }, en_attente: { bg: 'rgba(139,92,246,.08)', border: 'rgba(139,92,246,.2)', color: '#E8543A', label: 'En attente', icon: 'solar:clock-circle-bold-duotone' } };
    ...
    if (subTotal > 0) {
      body += '<div style="margin-bottom:16px;">'
        ...
        + '.tl-card:hover{border-color:var(--pilote-blue);box-shadow:0 2px 10px rgba(232,84,58,.12);}'
    ...
    let delegateBtn = '';
    if (canDelegate) {
      delegateBtn = '<button class="btn btn-sm" style="color:#E8543A;" onclick="TachesPage._openDelegateForm(\'' + t.id + '\')">'
  ...
  _openDelegateForm(taskId) {
    ...
    Modal.open({
      title: '<iconify-icon icon="solar:hand-shake-bold-duotone" style="color:#E8543A;"></iconify-icon> Déléguer la tâche',
      body: '<div style="padding:4px;">'
        ...
        + '<iconify-icon icon="solar:info-circle-line-duotone" style="color:#E8543A;"></iconify-icon> '
        ...
        + '</div>',
      footer: '<button class="btn" style="background:#E8543A;color:white;border:none;" onclick="TachesPage._confirmDelegate(\'' + taskId + '\')"><iconify-icon icon="solar:hand-shake-bold-duotone"></iconify-icon> Déléguer</button>'
  ...
  _getStyleContent() {
    return `
      ...
      .taches-tab.active {
        background:linear-gradient(135deg, rgba(232,84,58,.2), rgba(232,84,58,.12));
        color:#8AA8FF; font-weight:600;
        box-shadow:0 2px 8px rgba(232,84,58,.2), inset 0 1px 0 rgba(255,255,255,0.05);
      }
      ...
      .taches-fab {
        ...
        background:linear-gradient(135deg, #E8543A, #D23E22); color:#fff; border:none;
        cursor:pointer; display:flex; align-items:center; justify-content:center;
        box-shadow:0 4px 15px rgba(232,84,58,.4), 0 0 30px rgba(232,84,58,.15); transition:all .25s cubic-bezier(.4,0,.2,1); z-index:100;
      }
      .taches-fab:hover { transform:scale(1.08) rotate(90deg); box-shadow:0 8px 25px rgba(232,84,58,.5); }
      ...
      .dash-kpi-card::before {
        ...
        background:var(--kpi-accent, #E8543A); opacity:0; transition:opacity .2s;
      }
      ...
      .dash-bar-fill { height:100%; border-radius:4px; background:linear-gradient(90deg, #E8543A, #D23E22); transition:width .3s ease; }
      ...
      .kanban-col-body.drag-over { background:rgba(232,84,58,.08); border-radius:0 0 12px 12px; }
      ...
      .kanban-add-btn:hover { color:#E8543A; background:rgba(232,84,58,.06); }
      ...
      .kanban-tag {
        background:rgba(232,84,58,.1); color:#8AA8FF; padding:1px 7px; border-radius:8px;
      ...
      .eisen-quadrant.drag-over { outline:2px dashed rgba(232,84,58,.4); }
      .eisen-q-header {
      ...
      .gantt-view-btn.active { background:rgba(232,84,58,.15); color:#8AA8FF; border-color:rgba(232,84,58,.3); font-weight:600; }
      ...
      .gantt-months {
        display:flex; background:rgba(232,84,58,0.08);
        border-bottom:1px solid rgba(255,255,255,0.04);
      }
      .gantt-month {
        padding:6px 10px; font-size:12px; font-weight:600; color:#8AA8FF;
      ...
      .gantt-day.gantt-today-col { background:rgba(232,84,58,0.08); }
      .gantt-day.gantt-today-col .gantt-day-num { color:#8AA8FF; }
      ...
      .gantt-cell.gantt-today-col { background:rgba(232,84,58,0.04); }
      ...
      .gantt-today-line {
        position:absolute; top:0; width:2px; background:#8AA8FF;
        z-index:3; pointer-events:none; opacity:.7;
      }
      .gantt-today-line::before {
        content:''; position:absolute; top:-4px; left:-4px;
        width:10px; height:10px; border-radius:50%; background:#8AA8FF;
      }
      ...
      .reunion-date-day { font-size:1.2rem; font-weight:700; color:#E8543A; line-height:1; }
      .reunion-date-month { font-size:10px; color:#E8543A; text-transform:uppercase; font-weight:600; }
      ...
      .liste-bulk-bar {
        ...
        background:rgba(232,84,58,.08); border:1px solid rgba(232,84,58,.15); margin-bottom:12px;
