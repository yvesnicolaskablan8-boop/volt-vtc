...
const VehiculesPage = {
  ...
  _getFormFields() {
    ...
    return [
      ...
      { type: 'row-end' },
      { type: 'row-start' },
      { name: 'chauffeurAssigne', label: 'Chauffeur titulaire', type: 'select', placeholder: 'Sélectionner...', options: chauffeurs.filter(c => c.statut === 'actif').map(c => ({ value: c.id, label: `${c.prenom} ${c.nom}` })) },
      { name: 'doublureId', label: 'Doublure attitrée (remplaçant)', type: 'select', placeholder: 'Aucune doublure', options: chauffeurs.filter(c => c.statut === 'actif').map(c => ({ value: c.id, label: `${c.prenom} ${c.nom}` })) },
      { type: 'row-end' },
      { type: 'row-start' },
