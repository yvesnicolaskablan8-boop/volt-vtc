...
const DashboardPage = {
      ...
      <div class="yango-section" id="yango-section">
        ...
        <div class="grid-4" id="yango-kpis">
          <div class="kpi-card yango-kpi">
            ...
            <div class="kpi-label">Chauffeurs en ligne</div>
        ...
        <div class="card yango-drivers-card" id="yango-drivers-card">
          <div class="card-header">
            <span class="card-title"><i class="fas fa-users"></i> Chauffeurs Yango</span>
          </div>
          <div id="yango-drivers-table">
            <div class="yango-loading">
              <i class="fas fa-spinner fa-spin"></i> Chargement des donnees Yango...
            </div>
          </div>
        </div>
      </div>
  ...
  _renderYangoKPIs(stats) {
    ...
    if (coursesEl) {
      ...
      if (coursesDetail) {
        coursesDetail.innerHTML = enCours > 0
          ? `<span class="yango-dot yango-dot-green"></span> ${enCours} en cours`
          : `<i class="fas fa-check"></i> ${stats.courses?.mois || 0} ce mois`;
      }
    }
    ...
    if (!drivers || drivers.length === 0) {
      container.innerHTML = `
        <div class="yango-empty">
          ...
          <span>Aucun chauffeur enregistre sur Yango</span>
        </div>
      `;
      return;
    }
    ...
    container.innerHTML = `
      <table class="table">
        ...
        <tbody>
          ${sorted.map(d => {
            const statusConfig = {
              en_ligne: { label: 'En ligne', class: 'yango-status-online', icon: 'fa-circle' },
              occupe: { label: 'Occupe', class: 'yango-status-busy', icon: 'fa-circle' },
              hors_ligne: { label: 'Hors ligne', class: 'yango-status-offline', icon: 'fa-circle' }
            };
              ...
              <tr>
                <td>
                  <div style="display:flex; align-items:center; gap:8px;">
                    ...
                    <span style="font-weight:500;">${d.nom || 'Inconnu'}</span>
                  </div>
                </td>
                <td>
                ...
                <td class="tabular-nums">${parseFloat(d.balance || 0).toLocaleString('fr-FR')} FCFA</td>
