...
const AnalyseVersementsPage = {
    ...
    return `
      ...
      <div class="page-header av-header">
        ...
        <div class="page-actions">
          <label class="av-datebtn" title="Choisir un jour précis">
            <iconify-icon icon="solar:calendar-bold-duotone"></iconify-icon>
            <span class="av-datebtn-lbl">${this._anchor ? Utils.escHtml(Utils.formatDate(this._anchor)) : "Aujourd'hui"}</span>
            <iconify-icon icon="solar:alt-arrow-down-linear" class="av-datebtn-chev"></iconify-icon>
  ...
  _styles() {
    return `<style>
      ...
      .av-datebtn { position:relative; display:inline-flex; align-items:center; gap:8px; padding:8px 14px; background:var(--bg-secondary); border:1px solid var(--border-color); border-radius:20px; cursor:pointer; color:var(--text-primary); font-weight:700; font-size:13px; transition:border-color .15s, box-shadow .15s; }
      .av-datebtn:hover { border-color:var(--pilote-blue); box-shadow:0 2px 10px -4px rgba(245,81,46,.4); }
      .av-datebtn > iconify-icon:first-child { color:var(--pilote-blue); font-size:17px; }
      .av-datebtn-lbl { white-space:nowrap; }
      .av-datebtn-chev { color:var(--text-muted); font-size:14px; }
      .av-datebtn input[type=date] { position:absolute; inset:0; width:100%; height:100%; margin:0; padding:0; border:none; opacity:0; cursor:pointer; }
      .av-datebtn input[type=date]::-webkit-calendar-picker-indicator { position:absolute; inset:0; width:100%; height:100%; margin:0; opacity:0; cursor:pointer; }
