...
const ChauffeursPage = {
  ...
  _viewDocument(type, mimeType, chauffeurId) {
    ...
    if (doc.fichierType && doc.fichierType.startsWith('image/')) {
      Modal.open(`<iconify-icon icon="solar:eye-bold-duotone" style="color:#E8543A"></iconify-icon> ${doc.nom}`,
      ...
      async () => {
        ...
        try {
          ...
          Store.add('comptabilite', {
            ...
            reference: depId,
            notes: 'Créé automatiquement depuis recharge Yango',
            dateCreation: new Date().toISOString()
          });
  ...
  _showContraventions(chauffeurId) {
    ...
    Modal.open({
      ...
      body: `<div style="margin-bottom:16px;display:flex;gap:12px;">
        ...
        <div style="flex:1;background:rgba(232,84,58,.1);border:1px solid rgba(232,84,58,.2);border-radius:12px;padding:14px;text-align:center;">
          <div style="font-size:22px;font-weight:900;color:#E8543A;">${contraventions.length}</div>
  ...
  _showHistorique(chauffeurId) {
    ...
    absences.forEach(a => {
      ...
      timeline.push({
        ...
        color: a.type === 'suspension' ? '#ef4444' : a.type === 'maladie' ? '#f59e0b' : '#E8543A',
