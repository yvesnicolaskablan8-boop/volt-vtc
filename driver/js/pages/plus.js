...
const PlusPage = {
  async render(container) {
    const ligne = (route, icon, chipBg, iconColor, label, sousTitre) => `
      <button onclick="DriverRouter.navigate('${route}')" class="tap-scale" style="width:100%;display:flex;align-items:center;gap:16px;min-height:72px;padding:12px 16px;border-radius:1.25rem;border:1px solid var(--glass-border);background:var(--bg-card, rgba(255,255,255,0.04));cursor:pointer;font-family:inherit;margin-bottom:10px;text-align:left">
        <div style="width:52px;height:52px;border-radius:14px;background:${chipBg};color:${iconColor};display:flex;align-items:center;justify-content:center;flex-shrink:0">
    ...
    container.innerHTML = `
      ...
      ${ligne('signalements', 'solar:danger-triangle-bold-duotone', '#fee2e2', '#b91c1c', 'Signaler un problème', 'Panne, accident, souci…')}
      ${ligne('support', 'solar:phone-calling-bold-duotone', '#ecfdf5', '#047857', 'Appeler le bureau', 'On vous répond')}
